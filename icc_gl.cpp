...
#      endif
       ...
       int punkt_zahl = 0, index_zahl = 0;
       for (unsigned int i = 0; i < dreiecks_netze.size(); ++i)
	   {
         punkt_zahl += (int)dreiecks_netze[i].punkte.size();
         index_zahl += (int)dreiecks_netze[i].indexe.size();
	   }
       if(punkt_zahl > (int)netz.punkte.size())
         netz.punkte.reserve(punkt_zahl);
       /*if(index_zahl > netz.indexe.size())
         netz.indexe.(index_zahl);*/
       int punkte_n = 0;
       double abstand;
       for( j = 0; j < dreiecks_netze.size(); j++ )
       {
         ...
         {
           ICCnetzPunkt * begin = dreiecks_netze[j].punkte.begin();
           ICCnetzPunkt * end = dreiecks_netze[j].punkte.end();
           size_t diff = end - begin;
           size_t e = sizeof(ICCnetzPunkt);
           size_t n_p = diff/e;
       ...
       /*D*/ netz.indexe.insert(index_p);
           ...
           punkte_n += (int)dreiecks_netze[j].punkte.size();
         }
       }
      glPopMatrix();
...
#           endif
              glColor4d( netz.punkte[index[l]].farbe[0],
                         netz.punkte[index[l]].farbe[1],
                         netz.punkte[index[l]].farbe[2],
                         netz.punkte[index[l]].farbe[3] );
