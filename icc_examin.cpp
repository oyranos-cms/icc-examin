...
ICCexamin::nachricht( Modell* modell , int info )
{
  ...
  if(gl && info == GL_MOUSE_HIT3D)
  {
    ...
    if(profile.profil()->hasMeasurement())
    {
      ...
      LabToCIELab( &gl->mouse_3D_hit->lab[0], &lab[0], 1 );
      ...
      for(int i = 0; i < n; ++i)
      {
        len = fabs( dE( gl->mouse_3D_hit->lab, lab_v[i] ) );
      ...
      if(min < 5)
      {
        ...
        lab[1] = lab_v[min_pos].a*2.56-1.28;
        lab[2] = lab_v[min_pos].b*2.56-1.28;
        oyNamedColour_s * colour = oyNamedColourCreate(
                              lab, 0,
                              (icColorSpaceSignature)0, 0, name.c_str(), 0,
                              name.c_str(),
      ...
      for(int i = 0; i < n_; i+=3*mult)
      {
        ...
        lab[1] = lab_dv[i+1];
        lab[2] = lab_dv[i+2];
        len = fabs( dE( gl->mouse_3D_hit->lab, &lab[0] ) );
        if(len < min)
        {
          min = len;
          min_pos = i/3/mult;
      ...
      if(min < 5)
      {
        ...
        lab_dv.resize(3);
        lab[0] = lab_dv[0];
        lab[1] = lab_dv[1]*2.56-1.28;
        lab[2] = lab_dv[2]*2.56-1.28;
